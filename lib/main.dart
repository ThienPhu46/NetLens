...
import 'package:mobile_scanner/mobile_scanner.dart'; // Thư viện QR mới
...
// --- MÀN HÌNH QUÉT QR CODE DÙNG MOBILE_SCANNER (ĐÃ FIX LỖI) ---
...
class _QrScannerScreenState extends State<QrScannerScreen> {
  final MobileScannerController controller = MobileScannerController(
    ...
    returnImage: false,
    // 📍 FIX LỖI: autoResume không còn tồn tại, đã bị xóa
  );
  ...
  void _handleBarcode(Barcode barcode) {
    ...
    // Lấy nội dung QR (giả định là IP)
    String? ipAddress = barcode.rawValue;

    if (ipAddress != null && ipAddress.isNotEmpty) {
      setState(() {
        isProcessing = true; // Ngăn chặn xử lý lặp lại
      });

      controller.stop(); // Dừng camera ngay lập tức

      // Chuyển sang màn hình AR và truyền IP
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          Expanded(
            flex: 4,
            // 📍 FIX LỖI: Dùng Stack để tạo Overlay thủ công
            child: Stack(
              children: [
                MobileScanner(
                  controller: controller,
                  onDetect: (capture) {
                    // Xử lý barcode đầu tiên được phát hiện
                    if (capture.barcodes.isNotEmpty) {
                      _handleBarcode(capture.barcodes.first);
                    }
                  },
                ),
                // Overlay khung quét (Thay thế tham số overlay không tồn tại)
                Center(
                  child: Container(
                    width: 250,
                    height: 250,
                    decoration: BoxDecoration(
                      // FIX LỖI DEPRECATED: Dùng .withOpacity() trực tiếp
                      border: Border.all(
                          color: Colors.red.withOpacity(0.5), width: 3),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    // Lớp trong suốt để không che khuất camera
...
// --- MÀN HÌNH AR CÓ THAM SỐ ĐẦU VÀO ---
...
class _ArSwitchScreenState extends State<ArSwitchScreen>
  ...
  void initState() {
    ...
    switchData =
        SwitchData(name: "Loading...", ip: _currentTargetIp, ports: []);

    _fetchSwitchDataFromApi();
  }

  // 🔥 HÀM GỌI API (Cập nhật target IP)
  Future<void> _fetchSwitchDataFromApi() async {
    final String apiUrl =
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print("✅ Đã load dữ liệu: ${switchData.name}");
      ...
      print("❌ Lỗi API ($e). Sử dụng dữ liệu giả lập.");
      _useFallbackData();
    }
  }

  // 🔥 HÀM PARSE DỮ LIỆU (CHỈ LẤY ifAlias CÓ DỮ LIỆU)
  SwitchData _parsePrometheusText(String rawText, String targetIp) {
    ...
    return SwitchData(name: switchName, ip: switchIp, ports: ports);
  }
  ...
  void _placeSwitch2D(
    ...
    String baseName = "SW_${DateTime.now().millisecondsSinceEpoch}";
    ...
    final linesNode = _createNode(
      ...
      position: vector64.Vector3(
          newPosition.x, newPosition.y, newPosition.z - 0.0005),
  ...
  void _toggleVlan() async {
    ...
    String baseName = "SW_Vlan_${DateTime.now().millisecondsSinceEpoch}";
    currentSwitchNodeName = "${baseName}_panel";
    currentLinesNodeName = "${baseName}_lines";
    currentLabelNodeName = "${baseName}_label";
    ...
    final linesNode = _createNode(
      name: currentLinesNodeName!,
      texture: _linesTextureCache,
      position: vector64.Vector3(lastStablePosition!.x, lastStablePosition!.y,
          lastStablePosition!.z - 0.0005),
      size: vector64.Vector3(_switchWidth, _linesHeight, 0.0001),
    );
    final labelPosition = vector64.Vector3(
  ...
  void _resetScanning() {
    // Quay về màn hình QR để quét lại
    Navigator.of(context).pushReplacement(
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Thanh trạng thái (Chỉ hiện khi chưa tìm thấy)
          if (!frameVisible)
            Positioned(
              top: 50,
              left: 0,
  ...
  Future<Uint8List> _createLabelTexture(
    ...
    canvas.drawParagraph(paragraphLine1, ui.Offset(0, pixelHeight * 0.2));
    ...
    final ui.Paragraph paragraphLine2 = pbLine2.build()
      ..layout(ui.ParagraphConstraints(width: pixelWidth.toDouble()));
    canvas.drawParagraph(paragraphLine2, ui.Offset(0, pixelHeight * 0.55));
